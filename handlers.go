...
const (
	// 'iota' set value automatically
	ChannelStop = iota // will be 0
	UserStop           // will be 1
	MessageStop        // will be 2
)
...
func subscribeChannel(client *Client, data interface{}) {
	...
	go func() {
		// 'change' response will have two values
	...
	for {
		...
		case val := <-change:
			if val.NewValue != nil && val.OldValue == nil {
				eventName = changeEventName + " add" // "user add"
				data = val.NewValue
			} else if val.NewValue == nil && val.OldValue != nil {
				eventName = changeEventName + " remove" // "user remove"
				data = val.OldValue
			} else if val.NewValue != nil && val.OldValue != nil {
				eventName = changeEventName + " edit" // "user edit"
				data = val.NewValue
			}
...
// This implementation was native and resulted in Goroutine Leak
// Root problem:
//   I started a Goroutine for a client but I never stopped it
//   after the client disconnected
// How do I interrupt or stop the Goroutine when it's no longer needed.
// Essentially we need to pass a message into the Goroutine telling it to stop
// -------------------------------------------------------------------------
//func subscribeChannel(client *Client, data interface{}) {
//	go func() {
//		cursor, err := r.Table("channel").
